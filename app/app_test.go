...
func TestHTTPInfrastructure(t *testing.T) {
	cfg := Config{Environment: "test", AuthMode: "dev", DevToken: "local-development-token", DevSubject: "alice", RequestTimeout: time.Second}
	authenticator, err := identity.NewDevelopment(cfg.DevToken, cfg.DevSubject)
	if err != nil {
		t.Fatal(err)
	}
