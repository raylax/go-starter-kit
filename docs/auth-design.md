...
| `oauth_start.go`、`federation.go` | 按用途独立启动 OAuth、回调校验与事务完成逻辑 |
| `authentication_result.go` | 回调与重新认证的结果构造及读取契约 |
| `store.go`、`store_queries.go`、`store_flows.go` | 写入校验、邮件入队、显式查询集合和原子流程状态转换 |
| `notifications.go` | 验证邮件与安全通知的内容构造、收件规则和有效期 |
| `session.go`、`profile.go` | 会话校验与管理、资料读取与更新 |
| `audit.go` | 具名审计事件、事务内成功审计及独立失败记录 |
...
邮件内容与收件策略集中在 `notifications.go`，通过当前事务的 store 入队；store 只负责载荷校验与持久化。首次设置密码与修改密码统一通知“账户密码已修改”；注册完成和密码重置使用各自文案。密码安全通知说明已发生的操作及非本人操作时的处理方式；注册完成通知提示用户可以登录。

修改联系邮箱要求重新认证，在原会话和授权记录上绑定一个 5 分钟挑战。确认时复核会话、版本、目标邮箱和授权，再更新邮箱并撤销旧会话。若该邮箱仅被未完成注册的 pending 用户占用，事务内可以释放该占位：原用户必须尚未验证邮箱、未开启恢复且没有任何登录账号；清空邮箱并递增认证版本，使原注册挑战失效，然后将邮箱写入已完成验证的当前用户。原用户记录保留，不合并身份；已激活、已验证或已有登录账号的邮箱仍返回冲突。注册完成与邮箱收回通过条件更新及用户锁保持一致，并发冲突可以重试。安全通知发往原已验证联系邮箱。
...
`auth_flows.authenticated_at` 记录本应用完成证明校验的时间，与密码重新认证采用相同语义。提供商适配器只返回已验证的身份，不声明 GitHub 未提供的用户认证时间；OAuth 绑定和重新认证成功时由账户模块写入本地时间。
